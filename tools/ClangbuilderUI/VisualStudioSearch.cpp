...
bool PutEnvironmentVariableW(const wchar_t *name, const wchar_t *va)
{
	...
	if (dwSize >= PATHCCH_MAX_CCH) return false;
	if (buffer[dwSize - 1] != ';') {
		buffer[dwSize] = ';';
		dwSize++;
		buffer[dwSize] = 0;
...
/// HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Microsoft\VisualStudio\SxS\VS7 11.0 12.0 14.0
...
bool VisualStudioExists(const std::wstring &id)
{
	...
	if (RegGetValueW(hInst, nullptr, id.data(), RRF_RT_REG_SZ, &type,
