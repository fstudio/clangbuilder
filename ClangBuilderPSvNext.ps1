...
{
  Invoke-Expression -Command "cmake ..\llvm -G `"Visual Studio ${BDVSV} Win64`" -DLLVM_USE_CRT_MINSIZEREL:STRING=${BDCRT} -DLLVM_USE_CRT_RELEASE:STRING=${BDCRT} -DCMAKE_BUILD_TYPE=${BDTYPE} -DCMAKE_INSTALL_CONFIG_NAME=${BDTYPE} -DLLVM_APPEND_VC_REV:BOOL=ON "
  ...
  Invoke-Expression -Command "cmake ..\llvm -G `"Visual Studio ${BDVSV} ARM`" -DLLVM_USE_CRT_MINSIZEREL:STRING=${BDCRT} -DLLVM_USE_CRT_RELEASE:STRING=${BDCRT} -DCMAKE_BUILD_TYPE=${BDTYPE} -DCMAKE_INSTALL_CONFIG_NAME=${BDTYPE} -DLLVM_APPEND_VC_REV:BOOL=ON "
  Invoke-Expression -Command "msbuild /nologo LLVM.sln /t:Rebuild /p:Configuration=${BDTYPE} /p:Platform=ARM"
}ELSE{
Invoke-Expression -Command "cmake ..\llvm -G `"Visual Studio ${BDVSV}`" -DLLVM_USE_CRT_MINSIZEREL:STRING=${BDCRT} -DLLVM_USE_CRT_RELEASE:STRING=${BDCRT} -DCMAKE_BUILD_TYPE=${BDTYPE} -DCMAKE_INSTALL_CONFIG_NAME=${BDTYPE} -DLLVM_APPEND_VC_REV:BOOL=ON "
