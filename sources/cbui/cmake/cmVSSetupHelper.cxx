...
std::wstring VSInstanceInfo::GetInstallLocation() const {
  return VSInstallLocation;
}

cmVSSetupAPIHelper::cmVSSetupAPIHelper(unsigned int version)
    : Version(version), setupConfig(NULL), setupConfig2(NULL),
      setupHelper(NULL), initializationFailure(false) {
...
bool cmVSSetupAPIHelper::SetVSInstance(std::wstring const &vsInstallLocation) {
...
bool cmVSSetupAPIHelper::GetVSInstanceInfo(
  ...
  {
    std::wstring const vcRoot = vsInstanceInfo.GetInstallLocation();
    std::wstring vcToolsVersionFile =
        vcRoot + L"/VC/Auxiliary/Build/Microsoft.VCToolsVersion.default.txt";
    std::string vcToolsVersionA;
    std::ifstream fin(vcToolsVersionFile.c_str());
    if (!fin || !cmsys::GetLineFromStream(fin, vcToolsVersionA)) {
      return false;
    }
    std::wstring vcToolsVersion = cmsys::utf8towide(vcToolsVersionA);
    vcToolsVersion = cmsys::TrimWhitespace(vcToolsVersion);
    std::wstring const vcToolsDir =
        vcRoot + L"/VC/Tools/MSVC/" + vcToolsVersion;
    if (!cmsys::FileIsDirectory(vcToolsDir)) {
      return false;
    }
...
bool cmVSSetupAPIHelper::GetVSInstanceInfo(std::wstring &vsInstallLocation) {
...
bool cmVSSetupAPIHelper::GetVCToolsetVersion(std::wstring &vsToolsetVersion) {
...
bool cmVSSetupAPIHelper::IsEWDKEnabled() {
  ...
  cmsys::GetEnv(L"EnterpriseWDK", envEnterpriseWDK);
  cmsys::GetEnv(L"DisableRegistryUse", envDisableRegistryUse);
  return (cmsys::UnCaseEqual(envEnterpriseWDK, L"True") &&
          cmsys::UnCaseEqual(envDisableRegistryUse, L"True"));
}
